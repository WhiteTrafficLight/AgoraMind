...
class ChatService {
    ...
    try {
      ...
      try {
        const messagesResponse = await fetch(
          `/api/messages?roomId=${encodeURIComponent(roomId)}&action=getMessages`,
        );
        if (messagesResponse.ok) {
          ...
          if (
            ...
            room.messages = [...room.messages, ...uniqueLoadedMessages].sort(
            ...
            loggers.api.warn(
              'No messages found in chatMessages collection or invalid response format',
            );
          }
        } else {
      ...
      if (room.initial_message) {
        ...
        if (room.initial_message.text && room.initial_message.text.trim() !== '') {
          ...
          if (
            room.initial_message.sender === 'Moderator' ||
          ...
          // System , Welcome ( NPC )
            ...
            room.initial_message.sender !== 'System' &&
      ...
      if (room.dialogueType === 'debate') {
        const hasModeratorMessage = room.messages.some(
          (msg: ChatMessage) =>
        ...
        if (hasModeratorMessage) {
          ...
          room.messages = room.messages.filter(
            (msg: ChatMessage) => !msg.id.startsWith('temp-waiting-'),
          );
          ...
          const hasTempMessage = room.messages.some((msg: ChatMessage) =>
            msg.id.startsWith('temp-waiting-'),
          );
          if (hasTempMessage) {
            loggers.api.warn(
              'Temporary waiting message still present - no moderator message found yet',
            );
          }
        }
      }
  ...
  async createChatRoom(params: ChatRoomCreationParams): Promise<ChatRoom> {
    ...
    try {
      ...
      if (newRoom.initial_message) {
        ...
        // (Moderator, isSystemMessage=true, role=moderator)
        if (
          newRoom.initial_message.sender === 'Moderator' ||
          ...
          if (newRoom.initial_message.text && newRoom.initial_message.text.trim() !== '') {
            const isDuplicate = newRoom.messages.some(
              (msg) =>
  ...
  async loadNpcDetails(npcIds: string[]): Promise<NpcDetail[]> {
    loggers.api.debug('Loading details for NPCs using static data only', {
      count: npcIds.length,
      npcIds,
    });
  ...
  async getAIResponse(roomId: string | number): Promise<ChatMessage> {
    ...
    try {
      ...
      const npcDescriptions =
        room.npcDetails
          ?.map((npc) => {
            ...
            if (npc.communication_style)
              description += `, Communication style: ${npc.communication_style}`;
      ...
      let respondingNpc = room.npcDetails?.find(
        (npc) => npc.name.toLowerCase() === data.philosopher.toLowerCase(),
      );

      if (!respondingNpc) {
        respondingNpc = room.npcDetails?.find(
          (npc) => npc.id.toLowerCase() === data.philosopher.toLowerCase(),
        );
      }
      ...
      loggers.api.info('Created message object (citations included)', {
  ...
  async saveInitialMessage(roomId: string | number, message: ChatMessage): Promise<boolean> {
    try {
      ...
      if (cachedRoom) {
        ...
        loggers.api.warn('Room not found in local cache - will depend on DB lookup', {
          roomId: roomIdStr,
        });
      }
