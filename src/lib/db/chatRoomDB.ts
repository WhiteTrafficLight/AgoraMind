  ...
  async getAllChatRooms(): Promise<ChatRoom[]> {
    try {
      ...
      const rooms = await db
        .collection<DBChatRoom>('chatRooms')
        .find({})
        .sort({ updatedAt: -1 })
        .toArray();
  ...
  async getChatRoomById(id: string): Promise<ChatRoom | null> {
    try {
      ...
      const messages = await db
        .collection<DBChatMessage>('chatMessages')
        .find({ roomId })
        .sort({ timestamp: 1 })
        .toArray();
  ...
  async createChatRoom(room: ChatRoom): Promise<ChatRoom> {
    try {
      ...
      if (room.id) {
        ...
        try {
          const existingRooms = await db.collection<DBChatRoom>('chatRooms').find({}).toArray();
  ...
  async updateChatRoom(roomId: string, updates: Partial<ChatRoom>): Promise<boolean> {
    try {
      ...
      if (updates.freeDiscussionSessionId)
        updateFields.freeDiscussionSessionId = updates.freeDiscussionSessionId;

      updateFields.updatedAt = new Date();

      const result = await db
        .collection<DBChatRoom>('chatRooms')
        .updateOne({ roomId: normalizedRoomId }, { $set: updateFields });
