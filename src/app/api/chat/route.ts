...
loggers.chat.info(
  'API Key source:',
  apiKey === envVars.OPENAI_API_KEY ? '.env.local file' : 'system environment variable',
);
loggers.chat.info(
  'API Key check:',
  apiKey ? `${apiKey.substring(0, 7)}...${apiKey.substring(apiKey.length - 4)}` : 'MISSING',
);
...
export async function POST(req: NextRequest) {
  try {
    ...
    loggers.chat.info(
      `Using API key from: ${headerApiKey ? 'request header' : 'environment variable'}`,
    );
    ...
    const {
      ...
      user_message,
      npcs,
      llm_provider: clientLlmProvider,
      llm_model: clientLlmModel,
    } = data;

    // (messages, roomId, topic, context, participants)
    if (!room_id) {
      return NextResponse.json({ error: 'Missing room_id in request' }, { status: 400 });
    }

    if (!user_message) {
      return NextResponse.json({ error: 'Missing user_message in request' }, { status: 400 });
    }
    ...
    try {
      ...
      if (!backendResponse.ok) {
        ...
        throw new Error(
          `Python backend API request failed with status ${errorStatus}: ${errorText}`,
        );
      }
