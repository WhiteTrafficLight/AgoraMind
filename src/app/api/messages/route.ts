...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!message || !message.text || !message.sender) {
      ...
      return NextResponse.json({ error: 'Message must include text and sender' }, { status: 400 });
    }
    ...
    try {
      ...
      loggers.chat.info(
        `✅ Message saved to chatMessages collection with _id: ${savedMessage._id}`,
      );
      ...
      loggers.chat.info(
        `📊 room "${normalizedRoomId}"'s total message count: ${totalMessages}items`,
      );
    } catch (dbError) {
